...
def invoke(data1,id_list) -> pd.DataFrame():
    ...
    feature_dict = {"fac1":df_mean([size_neut(feature,price_dict) for feature in price_feature_dict.values()]).replace(0,np.nan)}
    X_df = feature_dict["fac1"].stack(dropna=False)
    ...
    data ={'id':id_list,'y':list(X_df.values)}
